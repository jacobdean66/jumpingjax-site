-- Production bookings.span_days is bigint. Cast inclusive day offsets before
-- adding them to dates so cancelled-rental restore works on production.
...
begin
  ...
  v_span_days := greatest(coalesce(v_booking.span_days, 1), 1)::integer;
  ...
  foreach v_item in array v_items loop
    for v_lock_date in
      ...
      from generate_series(
        ...
        v_booking.event_date + (v_span_days - 1),
        interval '1 day'
      ) day
      order by day
    loop
  ...
  join public.bookings other
   ...
   and other.event_date <= v_booking.event_date + (v_span_days - 1)
   and other.event_date + ((greatest(coalesce(other.span_days, 1), 1) - 1)::integer) >=
       v_booking.event_date
   and (
