-- Restore a cancelled rental without a read/check/write race.
-- Prepared locally by the cancellation/restore change; do not apply without approval.
...
begin
  ...
  -- Respect inventory records that are currently disabled or hidden.
  select rii.slug into v_unavailable_item
  from public.rental_inventory_items rii
  where rii.slug = any(v_items)
    and (rii.is_active is false or rii.public_visible is false)
  order by rii.slug
  limit 1;
