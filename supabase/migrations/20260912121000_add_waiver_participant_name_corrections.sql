-- Editable waiver display names for staff search/check-in.
-- Additive only: signed waiver participant names and documents remain immutable.

create table if not exists public.waiver_participant_name_corrections (
...
create index if not exists waiver_participant_name_corrections_participant_created_idx
  on public.waiver_participant_name_corrections (participant_id, created_at desc, id desc);
create index if not exists waiver_participant_name_corrections_search_first_idx
  on public.waiver_participant_name_corrections (corrected_search_first_name);
create index if not exists waiver_participant_name_corrections_search_last_idx
  on public.waiver_participant_name_corrections (corrected_search_last_name);
create index if not exists waiver_participant_name_corrections_search_full_idx
  on public.waiver_participant_name_corrections (corrected_search_full_name);

drop trigger if exists prevent_waiver_participant_name_correction_mutation_trg
  on public.waiver_participant_name_corrections;
create trigger prevent_waiver_participant_name_correction_mutation_trg
  before update or delete on public.waiver_participant_name_corrections
  for each row
  execute function public.prevent_append_only_mutation();

alter table public.waiver_participant_name_corrections enable row level security;

drop policy if exists "No public waiver_participant_name_corrections access"
  on public.waiver_participant_name_corrections;
create policy "No public waiver_participant_name_corrections access"
  on public.waiver_participant_name_corrections for all to anon, authenticated
  using (false) with check (false);

create or replace function public.correct_waiver_participant_display_name_atomic(p_payload jsonb)
returns jsonb
language plpgsql
security definer
...
as $$
declare
  v_participant_id uuid := nullif(p_payload->>'participant_id', '')::uuid;
  ...
  v_participant public.waiver_participants%rowtype;
  v_latest public.waiver_participant_name_corrections%rowtype;
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended('waiver_participant_name:' || v_participant_id::text, 0));
  ...
  from public.waiver_participant_name_corrections
  ...
  insert into public.waiver_participant_name_corrections (
    id, participant_id, submission_id,
    original_first_name, original_last_name,
    corrected_first_name, corrected_last_name,
    reason, corrected_by_staff_id, created_at
  ) values (
    v_correction_id, v_participant.id, v_participant.submission_id,
  ...
  return jsonb_build_object(
    ...
    'submission_id', v_participant.submission_id,
...
create or replace function public.list_open_play_same_day_conflicts(
...
as $$
  select
    ...
    a.unit_price_cents,
    a.created_at
  from public.open_play_visit_attendees a
  join public.waiver_participants p on p.id = a.participant_id
  left join lateral (
    select *
    from public.waiver_participant_name_corrections c
    where c.participant_id = p.id
  ...
  where a.business_day_ymd = p_business_day_ymd
    and a.status = 'active'
    and a.participant_id = any(p_participant_ids)
  order by a.created_at, a.id;
$$;

revoke all on function public.correct_waiver_participant_display_name_atomic(jsonb) from public, anon, authenticated;
revoke all on function public.search_waiver_participants_for_staff(text, integer) from public, anon, authenticated;
revoke all on function public.list_open_play_same_day_conflicts(text, uuid[]) from public, anon, authenticated;
grant execute on function public.correct_waiver_participant_display_name_atomic(jsonb) to service_role;
grant execute on function public.search_waiver_participants_for_staff(text, integer) to service_role;
grant execute on function public.list_open_play_same_day_conflicts(text, uuid[]) to service_role;
