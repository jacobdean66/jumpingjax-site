...
| D12 Wave 4 | OAuth admin diagnostics (`/admin/social-posts/publication-execution`) | Campaign Manager (name reused; orchestration not built) | Request, callback outcome, and session replay summaries surfaced on the existing admin page; GET-only, read-only, non-authoritative, and diagnostic-only; no OAuth, routes, code exchange, tokens, credentials, HTTP/fetch, API clients, publishing, or execution |
