...
D12 started with the integration planning gate. D12 Wave 1 adds secretless OAuth request modeling, and D12 Wave 2 adds secretless OAuth callback outcome modeling only. `docs/D12_INTEGRATION_GATE.md` remains the approval boundary before any real platform integration can begin. The Wave 1 code models authorization request intent, redirect/callback expectations, scope vocabulary validation, and replayable diagnostics. The Wave 2 code models callback outcomes including success intent, denial, cancellation, provider error, state mismatch, and expiry diagnostics. OAuth implementation, callback routes, authorization code exchange, credentials storage, secrets/tokens, HTTP/fetch to platform APIs, real redirects, real platform SDK integrations, publish, and execution automation remain explicitly forbidden until a later approved milestone.
