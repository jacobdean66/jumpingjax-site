...
- Asset semantics are now explicit: catalog rentals are `product` assets, invitation art is `theme-artwork`, and future owner-approved real event photos may be marked `lifestyle`. Product assets forbid synthesizing people; lifestyle assets preserve the people and event visible in the approved source. The UI identifies product versus approved lifestyle choices and explains the rule.
