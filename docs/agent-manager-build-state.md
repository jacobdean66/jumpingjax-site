...
## Agent reliability library checkpoint (2026-09-01)

- Added `fast-check` and `msw` as development-only dependencies. A fixed-seed, 250-case composite-booking property test now proves deterministic replay, correction identity changes, cancellation safety, atomic conflict blocking, and zero write permission. MSW intercepts the WhatsApp media-bridge contract in-process and fails on any unhandled network request, so the provider tests cannot contact Meta, Google, Resend, OpenAI, Supabase production, or another live service.
- Added `react-hook-form` and `@hookform/resolvers` as runtime dependencies and applied them only to the existing owner-only Answering Machine review form. Seven duplicated field-state handlers were replaced with one bounded Zod-backed form while the existing server-side validation, owner authentication, optimistic revision check, approval gate, and booking/calendar/payment/message isolation remain unchanged.
- Extracted the media-bridge fetch boundary into a small reusable adapter. The production route still requires HTTPS, a bearer secret, an 8-second timeout, signed Meta webhook verification, and fail-closed error handling.
- Validation: the focused 13/13 Booking/Answering Machine slice and the full 73/73 Agent Manager suite pass, including 250 deterministic generated scenarios and two isolated provider-contract tests; full TypeScript, focused ESLint with zero warnings, and the Next.js 16 production build pass. Sentry was not installed or configured, and no telemetry, live network request, credential, customer message, booking, calendar, payment, or social publication occurred.

## Permanent Agent website-supervisor checkpoint (2026-09-01)

- Reused the existing owner authentication, Agent Manager job/event ledger, approvals, pause/resume controls, emergency stop, booking and waiver triage, booking follow-up planner, rental catalog, Answering Machine readiness, Security Control Center, Supabase service-role boundary, and Vercel cron secret. No duplicate manager, conversation database, agent registry, security provider, or orchestration service was created.
- Added a real owner-only Permanent Agent conversation panel to `/admin/agents`. The bounded text box keeps durable question/reply history in the existing supervisor job ledger and rejects pasted password, token, API-key, or secret assignments. Replies are deterministic and use zero model calls.
- The supervisor checks four public website routes, recent Agent Manager failures, approvals, booking integration failures, active rental/facility counts, coordinated booking intents, the rental catalog, Answering Machine review/failure state, deployment identity, and existing Aikido/AITHURA code-security status. Only operational counts and redacted summaries enter the conversation snapshot; customer name, phone, email, address, payment, and message content are not selected.
- Exact owner commands can pause/resume a named specialist, enable/release emergency stop, run the existing bounded Booking triage, run Booking follow-up review, or run Waiver triage. Ambiguous production-change requests do not execute. Code, content, booking, calendar, customer-message, payment, deletion, credential, and deployment mutations remain behind their existing explicit approval boundaries.
- Added `/api/cron/agent-supervisor-watch` on the existing five-minute Vercel schedule. It is protected by the existing `CRON_SECRET`, records one daily durable result per changing issue fingerprint, and performs zero business/model writes. Repeated unchanged health or issue state deduplicates instead of filling the queue.
- Validation: 7/7 focused supervisor tests and the full 80/80 Agent Manager suite pass; full TypeScript, focused ESLint with zero warnings, and the Next.js 16.3 production build pass. The build contains `/admin/agents`, `/api/admin/agents/supervisor-chat`, and `/api/cron/agent-supervisor-watch`. No schema migration, provider credential, customer message, booking/calendar/payment mutation, website-content change, code repair, or production deployment was performed by this local checkpoint.

## Permanent Agent connection-audit repair checkpoint (2026-09-01)

- Reproduced a real routing defect with the owner prompt to make a Sonic-themed ad: because the request also mentioned a rental/party, the supervisor returned Booking/Rental counts instead of handing the work to Social Agent. The live Social Posts library already contained an owner-review Sonic draft, so creating another would also have duplicated completed work.
- Added one explicit wiring manifest for all eight registered agents. The dashboard now separates verified connection state from database runtime state, identifies Nomination as setup-required while production inbound is disabled, identifies Coding as not connected, identifies Health/Security and Booking/Waiver as read-only, identifies Social as staged, and removes pause buttons from cards that do not have a pauseable worker.
- Creative/ad/post intent now routes before rental and party keywords. A creation request searches existing draft/approved Social posts for an exact theme match; a match returns a clickable existing-draft handoff, while a non-match opens the existing staged Social workflow with bounded goal/theme fields prefilled. Neither path publishes, schedules, generates paid media, or creates a duplicate.
- Supervisor chat requests now carry a client request ID. The durable job is atomically claimed before any exact control runs, so browser/network replay returns the existing result or fails as already-processing instead of repeating pause, scan, or emergency actions. Non-pauseable specialists are no longer accepted by the text-control parser.
...
## Automatic rental route-planning checkpoint (2026-09-04)

- Reused the existing Route Planner, Google Routes matrix integration, rental lifecycle routes, owner editor, and trailer assignments. No duplicate planner, map provider, database table, paid service, or new dependency was added.
- Added a bounded Permanent Agent route-planning worker that runs after a rental is created, edited, confirmed, rejected, or cancelled. A reschedule replans both the old and new dates; duplicate dates are removed; one failed date does not prevent the other date from being planned; booking responses are not blocked by background routing.
- Corrected the real trailer capacities everywhere the active planner enforces them: Short Trailer (`truck-1`) holds 3 inflatables and Long Trailer (`truck-2`) holds 4. Automatic load numbering uses the same per-trailer capacities, while the existing drag-and-drop editor remains available for owner changes closer to the delivery date.
- The deterministic assignment protects setup deadlines first, then prefers the shorter available travel leg using the existing Google route matrix. Missing matrix data retains the existing bounded fallback instead of inventing mileage. Customer contact details are not written to planner logs.
...
## Route-planner legacy backfill production evidence (2026-09-04)

- PR #107 passed both GitHub/Vercel checks and merged to `main` as `8919e5d9608b3b5a8f8e20997d6e4dcec62de78a`. The signed-in production Route Planner exposed the bounded multi-date auto-plan control.
- The first production attempt failed closed with zero writes because the optional Google Routes matrix provider rejected all nine affected dates. PR #108 passed both checks and merged as `515b7c3`; the planner now retains its existing deterministic bounded drive-time fallback when that optional provider is unavailable and omits mileage notes rather than inventing distance.
- The owner-confirmed 90-day backfill ran in three 30-day windows covering 2026-09-04 through 2026-12-02. The windows planned 86 routes across 9 affected dates, 36 routes across 4 affected dates, and 10 routes across 2 affected dates (132 delivery/pickup route assignments across 15 affected dates).
- Post-run production audits showed `Auto-plan 0 dates`, `No unassigned drop-offs`, and `No unassigned pickups` in every window. Only route-assignment fields were changed; no booking, customer, calendar, payment, message, credential, provider, dependency, or paid-plan mutation was made.

## WhatsApp native-voicemail test transport checkpoint (2026-09-04)

- Verified against Meta's current official Calling API documentation that a public test number can test calling without the production 2,000-recipient messaging limit, but calling must be enabled explicitly. Meta native voicemail is delivered through the standard `messages` webhook as an inbound audio message whose `messages[].id` is the originating call ID.
- Added a reversible `native_voicemail` test mode alongside the existing `interactive_bridge` mode. The signed webhook now recognizes only documented `wacid.*` voicemail audio events, deduplicates them into the existing private inbox, and retains the existing two-way bridge path unchanged.
- Added additive migration `20260904153000_add_whatsapp_native_voicemail.sql`. It stores only bounded media ID/type/hash metadata, preserves RLS and service-role-only writes, and extends owner review so a manually typed transcript can be marked complete before approval. It does not create a booking, calendar event, message, payment, or provider resource.
- Added an owner-authenticated, private/no-store audio proxy. It gets a fresh media URL server-side, allowlists Meta/WhatsApp hosts, bounds time and content size, and never exposes the access token or stored provider media ID to the browser. The dashboard displays playback only when a voicemail exists.
- Validation passes: 93/93 Agent Manager and Answering Machine tests, including ordinary-audio rejection and mocked Meta media download/unsafe-host rejection; full TypeScript passes; focused ESLint passes with zero errors; and the Next.js 16.3 production build passes with the new `/api/admin/answering-machine/[id]/audio` route. The local build required one worker because the desktop had only about 0.2 GB free RAM during verification.
- External state remains unchanged at this checkpoint. Meta is already signed in and the existing app exposes public test number `+1 (555) 204-3651`, phone-number ID `1287615654437974`, and test WABA ID `1745532420039179`, but no token exists. Next exact gate: create the scoped Meta test token, configure the signed webhook and `messages`/`calls` subscriptions, upload the voicemail announcement, enable native voicemail on the public test number, add the server-only Vercel variables, apply only the additive migration, deploy, and place one controlled test call.

## Permanent Agent continuous-operations checkpoint (2026-09-25)

- Replaced the dashboard's newest-40-job approximation with exact queued, claimed, running, stale, and oldest-active queue measurements. The manager now distinguishes a fresh watcher from a stale watcher and raises stale work older than 15 minutes as a critical condition.
- Added a canonical read-only coverage registry for all 20 owner-dashboard services. The supervisor and dashboard now expose connected, read-only, degraded, setup-required, and unavailable states without claiming provider access that is not configured.
- Added a protected, bounded Agent Manager worker cron that processes at most eight durable jobs every five minutes. Duplicate watcher runs refresh the manager heartbeat without adding noisy ledger events, so work can drain continuously and watcher freshness remains visible.
- Added direct service-coverage answers in the supervisor chat and automatic dashboard refresh after a chat action. The owner can inspect every service connection, active queue state, stale work, and the oldest active job from one screen.
- Validation passes: 96 focused Agent Manager tests, full TypeScript, focused ESLint, `git diff --check`, and the Next.js 16.3 production build with all 154 pages and `/api/cron/agent-worker` present.
- Scope/safety: this checkpoint is ready for publication. It creates no booking, customer message, calendar event, payment, provider credential, or paid service. A real Answering Machine test call still requires the already-documented Meta credential, webhook subscription, native-voicemail enablement, Vercel variables, and additive migration.
