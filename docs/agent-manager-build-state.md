...
- Remaining DEV proof gate: the local app process still needs the private Trigger.dev Development key to run the Step 2 fixture against the DEV worker. Owner must provide it through a secret-safe local handoff; then run `Prove source dedupe` and verify `/admin/giveaway` plus `/nominees`.
