...
**D13–D18 execution plan:** `docs/D13_D18_EXECUTION_PLAN.md` defines remaining milestones. D13 (credential architecture) and D15 Waves 1–3 (orchestration/planning/eligibility preflight) are implemented as contract, persistence, and diagnostic layers only. D14 (live OAuth), D15 live HTTP adapters, D16 (controlled execution), D17 (production operations), and D18 (autonomous Campaign Manager) remain not started. `docs/D12_INTEGRATION_GATE.md` remains the approval boundary before live OAuth, HTTP, or execution.

**Not started (by design):** scheduler execution, publisher execution, Metrics collection from real services, Learning production store and automation, live OAuth/callback routes, authorization code exchange, live token storage, HTTP/fetch to social platform APIs, real platform SDK integrations, publishing, execution runners, Campaign Manager orchestration, and background automation (cron, queues, workers, retry engines).
