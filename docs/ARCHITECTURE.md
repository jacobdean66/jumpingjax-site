...
### Layer 16: Credential Runtime Orchestration and Resolution Planning (D15 Wave 1-3, complete)
...
- **Publication Execution (D10 + D11 Wave 1-4 + D12 Wave 4 + D15 Wave 1-3 + H32/H34/H35/H36/H37/H38/H39/H40/H41/H42/H45/H46/H47/H48)** — durable Execution request/result records, computed replay, read-only preflight diagnostics, simulated planner visibility, adapter contract diagnostics, runbook readiness diagnostics, coordinator pipeline diagnostics, platform adapter registry diagnostics, Meta/TikTok/LinkedIn adapter contract diagnostics, credential/OAuth boundary diagnostics, platform readiness gate diagnostics, D12 OAuth request/callback/session replay diagnostics, D15 credential runtime orchestration diagnostics, D15 provider integration planning diagnostics, D15 credential resolution execution bridge diagnostics, and D15 publication execution eligibility preflight diagnostics through H31 bridge; no execution controls. D12 OAuth diagnostics are read-only and are not exposed as a mutation surface.
...
**D13–D18 planning (complete, not implemented):** `docs/D13_D18_EXECUTION_PLAN.md` defines credential vault, OAuth integration, platform API adapters, controlled execution, production operations, and autonomous Campaign Manager milestones. No D13+ code is authorized until that plan is reviewed and the corresponding milestone gate passes.

Completed implementation phases: D5 Working Context, D6 Publication Layer, D7 Publication Targets, D8 Publication Ledger, H1-H8 platform hardening, D9 Wave 1 scheduler foundation (M1-M3 library only), D9 Wave 2 scheduler durable storage (H9-H11), D9 Wave 3 scheduler read visibility/admin wiring (H12-H14), D9 Wave 4 Publisher foundation (M4 domain, M5 repository contract, M6 replay), D9 Wave 5 Publisher durable persistence (H15 SQL, H16 row mapping, H17 production store), D9 Wave 6 Publisher read integration (H18 bridge, H19 read-only admin, H20 navigation), D9 Wave 7 passive Metrics foundation (M7 domain, M8 repository contract, M9 replay), D9 Wave 8 Metrics durable read integration (H21 SQL, H22 row mapping, H23 production store, H24 bridge/admin/navigation), D9 Wave 9 passive Learning foundation (M10 domain, M11 repository contract, M12 replay), D9 Wave 10 Learning Read Layer (H25 fail-closed read bridge, H26 read-only admin, H27 navigation/explainability), D9 Wave 11 AI Operations Console (unified subsystem overview, cross-system pipeline explainability, passive diagnostics — composition over existing bridges only, no new bridge or persistence), D10 Wave 1 Execution boundary design (M1 domain, M2 repository contract, M3 replay — library-only, no execution or platform APIs), D10 Wave 2 Execution durable infrastructure (H28 SQL, H29 row mapping, H30 production store — durable persistence only, no execution), D10 Wave 3 Execution read visibility (H31 bridge, H32 read-only admin, H33 navigation — no execution, APIs, adapters, workers, cron, retries, or persistence changes), D10 Wave 4 Execution preflight gate (M4 preflight diagnostics, M5 replay integration, H34 admin visibility — read-only, no execution, no APIs, no adapters, no workers/cron/queues/retries), D10 Wave 5 Execution planner (M6 planner domain, M7 planner replay, H35 admin visibility — simulated-only, no execution, no APIs, no adapters, no platform integrations), D10 Wave 6 Execution adapter contract layer (M8 adapter contracts, M9 dry-run reference adapter, M10 adapter replay, H36 admin visibility — contract-only, only dry-run reference adapters, no real platform adapters, OAuth, credentials, or execution), D10 Wave 7 Execution runbook readiness layer (M11 runbook domain, M12 runbook replay, H37 admin visibility — human checklist, no automation), D10 Wave 8 Execution coordinator (M13 coordinator domain, M14 coordinator replay, H38 admin visibility — pipeline modeled end-to-end, no execution), and D11 Wave 1 Platform adapter architecture (M1 registry, M2 factory, M3 capability replay, H39 admin visibility — architecture only, no OAuth/credentials/HTTP/real execution).

Not started: D9 scheduler execution, D9 publisher execution, Publisher API routes, Metrics collection from real services, Learning persistence and a production learning store, Learning-layer automation and model training, OAuth implementation, callback route implementation, authorization code exchange, credentials storage, HTTP/fetch to platform APIs, real platform SDK integrations, publishing, execution runners, D10 real execution, D10 Campaign Manager orchestration, and all background automation (cron, queues, workers, retry engines). D10 Waves 1-8 complete the passive Execution modeling stack end to end; D11 Wave 1 adds platform adapter registry/factory/replay, D11 Wave 2 adds the Meta contract shell, D11 Wave 3 alt adds credential/OAuth boundaries and TikTok/LinkedIn contract shells, and D11 Wave 4 adds the platform readiness gate. D12 starts from `docs/D12_INTEGRATION_GATE.md`; D12 Wave 1 adds secretless OAuth request modeling, D12 Wave 2 adds secretless OAuth callback outcome modeling, D12 Wave 3 adds secretless OAuth session replay, and D12 Wave 4 adds read-only admin diagnostics for those replay summaries. Real adapters, OAuth implementation, credentials storage, secrets/tokens, HTTP/fetch to platform APIs, real redirects/callback routes, workers, cron, queues, retries, and execution automation remain absent by design until a later explicitly approved phase.

See `docs/ROADMAP.md` for milestone detail. D9 Wave 1 (M1-M3) provides intent and replay only. D9 Wave 2 (H9-H11) adds durable intent storage. D9 Wave 3 (H12-H14) makes that durable intent read-visible through a bridge and read-only admin page. D9 Wave 4 (M4-M6) adds the Publisher domain, contract, and replay helpers. D9 Wave 5 (H15-H17) adds durable Publisher persistence. D9 Wave 6 (H18-H20) makes Publisher records read-visible through a bridge and read-only admin page. D9 Wave 7 (M7-M9) adds passive Metrics domain, repository contract, and replay helpers. D9 Wave 8 (H21-H24) adds Metrics durable persistence, production store, bridge, read-only admin, and navigation. D9 Wave 9 (M10-M12) adds the passive, explainable Learning domain, repository contract, and replay helpers. D9 Wave 10 (H25-H27) adds a fail-closed Learning read bridge with no production store, a read-only explainable admin page, and navigation/cross-link wiring. D9 Wave 11 adds the AI Operations Console: a unified read-only overview, cross-system pipeline explainability, and passive diagnostics composed entirely from existing bridges and replay helpers, with no new bridge, persistence, or mutation surface. D10 Wave 1 (M1-M3) adds the passive Execution domain, repository contract, and replay helpers described in Layer 13 above — library-only, no execution, no platform APIs, no persistence, no bridge, no admin UI. D10 Wave 2 (H28-H30) adds durable Execution SQL, row mapping, and a production store as described in the Layer 13 continuation above — persistence only, still no execution. D10 Wave 3 (H31-H33) adds the fail-closed Execution bridge, read-only admin page, and navigation. D10 Wave 4 (M4-M5 + H34) adds a read-only preflight gate and admin diagnostics. D10 Wave 5 (M6-M7 + H35) adds a simulated-only planner and admin visibility. D10 Wave 6 (M8-M10 + H36) adds adapter contract vocabulary, dry-run reference adapters, adapter replay, and admin adapter diagnostics. No scheduler execution, publisher execution engine, Publisher API route, Metrics collection from real services, Learning persistence/production store, learning automation, real platform adapters, external APIs, platform integrations, workers, cron, queues, retries, or execution automation exists yet.
