...
- **M7 TikTok adapter contract** (`social-platform-tiktok-adapter.ts`) - TikTok Business Account channel support, TikTok post/media request shapes, validation, diagnostics, capability flags, and forbidden-state detection. Contract-only.
- **M8 LinkedIn adapter contract** (`social-platform-linkedin-adapter.ts`) - LinkedIn Company Page channel support, LinkedIn post/media request shapes, validation, diagnostics, capability flags, and forbidden-state detection. Contract-only.
- **M9-M10 TikTok/LinkedIn dry-run adapters** - simulate would-send summaries, media refs required, missing capabilities, and blocked reasons; wired to D11 factory/registry. No real platform calls.
- **M11-M12 TikTok/LinkedIn adapter replay** - computes platform-ready/blocked jobs, post-kind readiness buckets, missing media, unsupported channel, and missing capability jobs. Pure replay only.
...
- **Publication Execution (D10 + D11 Wave 1-2 + H32/H34/H35/H36/H37/H38/H39/H40)** — durable Execution request/result records, computed replay, read-only preflight diagnostics, simulated planner visibility, adapter contract diagnostics, runbook readiness diagnostics, coordinator pipeline diagnostics, platform adapter registry diagnostics, and Meta adapter contract diagnostics through H31 bridge; no execution controls.
...
| D11 Wave 3 alt (M7-M12 + H41) | TikTok/LinkedIn platform adapter contract shells (contract + dry-run + replay + admin visibility) | Campaign Manager (name reused; orchestration not built; contract shells only, no APIs/OAuth/credentials/publishing) |
| D11 Wave 4 (M13-M15 + H42) | Platform readiness gate (readiness domain + replay + admin visibility) | Campaign Manager (name reused; orchestration not built; read-only gate only, no OAuth/credentials/HTTP/real execution) |
