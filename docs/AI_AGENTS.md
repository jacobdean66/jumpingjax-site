...
## Implemented Platform Stack (D1-D11, H1-H42 + Wave 11 Operations Console)
...
**D12 integration gate and Waves 1-4:** `docs/D12_INTEGRATION_GATE.md` defines the approval criteria, security requirements, rollback requirements, human approval requirements, validation requirements, risk register, and admin diagnostics required before real platform integration can begin. D12 Wave 1 adds secretless OAuth request modeling (`social-platform-oauth-request.ts`) and replay diagnostics (`social-platform-oauth-request-replay.ts`) only. D12 Wave 2 adds secretless OAuth callback outcome modeling (`social-platform-oauth-callback.ts`) and replay diagnostics (`social-platform-oauth-callback-replay.ts`) only. D12 Wave 3 adds secretless OAuth session replay (`social-platform-oauth-session-replay.ts`) only. D12 Wave 4 surfaces those request, callback outcome, and session replay summaries on `/admin/social-posts/publication-execution` as GET-only, read-only, non-authoritative diagnostics only. D12 still does not implement real OAuth, routes, callback routes, authorization code exchange, credential storage, secrets/tokens, HTTP/fetch, SDKs/API clients, real redirects, publishing, execution runners, workers, cron, queues, or retries.

**D15 credential runtime Waves 1-3:** D15 Wave 1 adds credential runtime orchestration (`social-credential-runtime-orchestrator.ts` + replay) composing D13-D14 readiness, diagnostics, capability, audit, and provider-agnostic resolution flow modeling. D15 Wave 2 adds provider integration planning contracts (`social-provider-integration-planning.ts` + replay + orchestration compatibility) and the credential resolution execution bridge (`social-credential-resolution-execution-bridge.ts` + replay) for deterministic provider reference selection, repository-backed reference lookup, orchestration-aware resolution planning, and replay-compatible execution planning. D15 Wave 3 adds publication execution eligibility preflight (`social-publication-execution-eligibility-preflight.ts` + replay) composing D10 execution preflight with credential readiness, orchestration readiness, provider capability summaries, deterministic blocking reason aggregation, and append-only audit compatibility. H45-H48 admin diagnostics on `/admin/social-posts/publication-execution` remain GET-only, read-only, and non-authoritative. D15 still does not implement live OAuth, HTTP, SDKs, encryption execution, publishing, or execution authority.
...
| `/admin/social-posts/publication-execution` | D10 + D11 Wave 1-4 + D12 Wave 4 + D15 Wave 1-3 Execution records, replay, read-only preflight diagnostics, simulated planner visibility, adapter/runbook/coordinator diagnostics, platform adapter registry diagnostics, Meta/TikTok/LinkedIn adapter diagnostics, credential/OAuth boundary diagnostics, platform readiness gate diagnostics, OAuth request/callback/session replay diagnostics, D15 credential runtime orchestration diagnostics, D15 provider integration planning diagnostics, D15 credential resolution execution bridge diagnostics, and D15 publication execution eligibility preflight diagnostics via H31 bridge |
...
**D13–D18 execution plan (planning complete):** See `docs/D13_D18_EXECUTION_PLAN.md`. Campaign Manager orchestration, Publisher live execution, Analytics collection, and Learning persistence/automation remain **not started** until D16–D18 respectively; credential and OAuth work is scoped to D13–D14 only.
