...
| `/admin/social-posts/publication-execution` | D10 + D11 Wave 1-4 + D12 Wave 4 Execution records, replay, read-only preflight diagnostics, simulated planner visibility, adapter/runbook/coordinator diagnostics, platform adapter registry diagnostics, Meta/TikTok/LinkedIn adapter diagnostics, credential/OAuth boundary diagnostics, platform readiness gate diagnostics, and OAuth request/callback/session replay diagnostics via H31 bridge |
