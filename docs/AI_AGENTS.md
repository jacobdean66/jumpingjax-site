...
**D12 integration gate and Wave 1:** `docs/D12_INTEGRATION_GATE.md` defines the approval criteria, security requirements, rollback requirements, human approval requirements, validation requirements, risk register, and admin diagnostics required before real platform integration can begin. D12 Wave 1 adds secretless OAuth request modeling (`social-platform-oauth-request.ts`) and replay diagnostics (`social-platform-oauth-request-replay.ts`) only. D12 still does not implement real OAuth, credential storage, secrets/tokens, HTTP/fetch, SDKs/API clients, real redirects, callback routes, publishing, execution runners, workers, cron, queues, or retries.
