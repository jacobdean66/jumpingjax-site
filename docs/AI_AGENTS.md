...
D10 Wave 4, D10 Wave 5, and D10 Wave 6 extend that visibility: Wave 4 adds the read-only preflight gate, Wave 5 adds the simulated-only Execution Planner, and Wave 6 adds adapter contract vocabulary with dry-run reference adapters only. Execution still does not run and has no real platform adapters, OAuth, credentials, platform integrations, external APIs, HTTP/fetch, workers, cron, queues, retries, API routes, SQL changes, or persistence changes.
