...
**D15 credential runtime Waves 1-2:** D15 Wave 1 adds credential runtime orchestration (`social-credential-runtime-orchestrator.ts` + replay) composing D13-D14 readiness, diagnostics, capability, audit, and provider-agnostic resolution flow modeling. D15 Wave 2 adds provider integration planning contracts (`social-provider-integration-planning.ts` + replay + orchestration compatibility) and the credential resolution execution bridge (`social-credential-resolution-execution-bridge.ts` + replay) for deterministic provider reference selection, repository-backed reference lookup, orchestration-aware resolution planning, and replay-compatible execution planning. H45-H47 admin diagnostics on `/admin/social-posts/publication-execution` remain GET-only, read-only, and non-authoritative. D15 still does not implement live OAuth, HTTP, SDKs, encryption execution, publishing, or execution authority.
...
| `/admin/social-posts/publication-execution` | D10 + D11 Wave 1-4 + D12 Wave 4 + D15 Wave 1-2 Execution records, replay, read-only preflight diagnostics, simulated planner visibility, adapter/runbook/coordinator diagnostics, platform adapter registry diagnostics, Meta/TikTok/LinkedIn adapter diagnostics, credential/OAuth boundary diagnostics, platform readiness gate diagnostics, OAuth request/callback/session replay diagnostics, D15 credential runtime orchestration diagnostics, D15 provider integration planning diagnostics, and D15 credential resolution execution bridge diagnostics via H31 bridge |
