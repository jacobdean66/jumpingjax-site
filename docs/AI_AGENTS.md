...
**D12 integration gate and Waves 1-2:** `docs/D12_INTEGRATION_GATE.md` defines the approval criteria, security requirements, rollback requirements, human approval requirements, validation requirements, risk register, and admin diagnostics required before real platform integration can begin. D12 Wave 1 adds secretless OAuth request modeling (`social-platform-oauth-request.ts`) and replay diagnostics (`social-platform-oauth-request-replay.ts`) only. D12 Wave 2 adds secretless OAuth callback outcome modeling (`social-platform-oauth-callback.ts`) and replay diagnostics (`social-platform-oauth-callback-replay.ts`) only. D12 still does not implement real OAuth, callback routes, authorization code exchange, credential storage, secrets/tokens, HTTP/fetch, SDKs/API clients, real redirects, publishing, execution runners, workers, cron, queues, or retries.
...
| `/admin/social-posts/publication-execution` | D10 + D11 Wave 1-4 Execution records, replay, read-only preflight diagnostics, simulated planner visibility, adapter/runbook/coordinator diagnostics, platform adapter registry diagnostics, Meta/TikTok/LinkedIn adapter diagnostics, credential/OAuth boundary diagnostics, and platform readiness gate diagnostics via H31 bridge |
