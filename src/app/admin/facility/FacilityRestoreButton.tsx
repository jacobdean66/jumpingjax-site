...
export function FacilityRestoreButton({
  bookingId,
  customerName,
  partyTime,
  childName,
  ...
  async function restore() {
    ...
    try {
      const response = await fetch(
        `/api/admin/facility/${encodeURIComponent(bookingId)}/restore`,
      ...
      if (!response.ok || result.ok === false) {
        setFailed(true);
        setMessage(result.message ?? "The facility party could not be restored.");
        return;
      }
      setMessage(result.message ?? "Facility party restored to pending.");
      setOpen(false);
      router.refresh();
    } catch {
      setFailed(true);
      setMessage("Could not reach the server. The facility party remains cancelled.");
    } finally {
      setIsWorking(false);
    }
  }
        ...
        >
          ...
          >
            <h2 id={titleId} className="text-2xl font-black text-slate-950">
              Restore this facility party?
            </h2>
            <p className="mt-2 text-sm font-semibold text-slate-600">
              The server will check the original time slot. A conflict leaves
              this party cancelled.
            </p>
            ...
            <p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs font-bold text-amber-950">
              A successful restore returns the original party to pending approval.
            </p>
            {message && failed ? (
              <p
                role="alert"
