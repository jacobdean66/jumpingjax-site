...
import { calculateInvoiceTotals, type BookingInvoice } from "@/lib/invoices/shared";

function formatMoney(value: number): string {
  return new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(value);
}

export function InvoicePrintDocument({ invoice, copyLabel = "CUSTOMER COPY" }: { invoice: BookingInvoice; copyLabel?: string }) {
  ...
  return (
    <article className="mx-auto max-w-4xl bg-white p-8 shadow-xl print:max-w-none print:p-5 print:shadow-none">
      <header className="grid grid-cols-[1fr_180px] gap-5 border-t-8 border-sky-950 border-b-4 border-b-rose-500 py-5">
        ...
        <div className="relative overflow-hidden rounded-xl"><Image src="/hero.jpg" alt="Jumping Jax waterslide" fill sizes="180px" className="object-cover" /><div className="absolute right-0 top-0 text-right"><span className="inline-block bg-rose-500 px-3 py-2 text-lg font-black text-white">INVOICE</span><span className="block bg-white/95 px-2 py-1 text-[10px] font-black tracking-wider text-sky-950">{copyLabel}</span></div></div>
      </header>
