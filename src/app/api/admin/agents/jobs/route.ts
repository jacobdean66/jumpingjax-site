...
export async function POST(request:Request){
  ...
  try{const job=await enqueueJob({agentKey:body.agentKey,jobType:body.jobType,source:"admin.manual",idempotencyKey:body.idempotencyKey,actorId:auth.identity.id});const result=await runOne(`admin:${auth.identity.id}`);return Response.json({ok:true,job,result});}catch{return Response.json({ok:false,error:"The durable job could not be processed."},{status:503});}
}
