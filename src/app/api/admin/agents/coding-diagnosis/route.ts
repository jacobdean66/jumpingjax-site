import { runCodingDiagnosis } from "@/lib/agent-manager/coding-diagnosis-service";
...
export async function POST(request: Request) {
  ...
  try {
    return Response.json({ ok: true, result: await runCodingDiagnosis(auth.identity.id) });
  } catch (error) {
    const message = error instanceof Error && /paused|emergency stop/i.test(error.message)
      ? error.message
      : "Coding diagnosis could not be completed safely.";
