...
export async function applyLegacyVisitCorrection(options: {
  visitId: string;
  staffId: string;
  correction: CorrectionRequest;
  now?: Date;
  ...
  const { data, error } = await supabase.rpc(
    "apply_smartwaiver_legacy_visit_correction_atomic",
    { p_payload: payload },
  );
  if (error) throw new Error("Unable to apply Legacy correction");
  ...
  if (result.outcome !== "applied") {
    throw new LedgerValidationError(
      outcomeMap[result.outcome] || "Unable to apply Legacy correction",
    );
  }
